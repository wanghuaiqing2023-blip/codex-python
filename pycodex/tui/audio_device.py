...
RUST_MODULE = RustTuiModule(crate="codex-tui", module="audio_device", source="codex/codex-rs/tui/src/audio_device.rs")
...
class AudioDevice:
    ...
    default_input: SupportedStreamConfig | None = None
    default_output: SupportedStreamConfig | None = None
...
def _kind(kind: RealtimeAudioDeviceKind | str) -> RealtimeAudioDeviceKind:
...
def _default_host(host: AudioHost | None) -> AudioHost:
    if host is None:
        raise NotImplementedError("real audio device enumeration requires an injected AudioHost")
    return host


def list_realtime_audio_device_names(kind: RealtimeAudioDeviceKind | str, *, host: AudioHost | None = None) -> list[str]:
...
def select_configured_input_device_and_config(config: Any, *, host: AudioHost | None = None) -> tuple[AudioDevice, SupportedStreamConfig]:
    return select_device_and_config(RealtimeAudioDeviceKind.MICROPHONE, config, host=host)


def select_configured_output_device_and_config(config: Any, *, host: AudioHost | None = None) -> tuple[AudioDevice, SupportedStreamConfig]:
...
def preferred_input_config(device: AudioDevice) -> SupportedStreamConfig:
    candidates = []
    for range_ in device.supported_input_configs():
...
def configured_name(kind: RealtimeAudioDeviceKind | str, config: Any) -> str | None:
...
def find_device_by_name(host: AudioHost, kind: RealtimeAudioDeviceKind | str, name: str | None) -> AudioDevice | None:
    if name is None:
        return None
    for device in devices(host, _kind(kind)):
        try:
            if device.name() == name:
                return device
        except Exception:
...
def devices(host: AudioHost, kind: RealtimeAudioDeviceKind | str) -> list[AudioDevice]:
    return host.input_devices() if _kind(kind) is RealtimeAudioDeviceKind.MICROPHONE else host.output_devices()


def default_device(host: AudioHost, kind: RealtimeAudioDeviceKind | str) -> AudioDevice | None:
    return host.default_input_device() if _kind(kind) is RealtimeAudioDeviceKind.MICROPHONE else host.default_output_device()


def default_config(device: AudioDevice, kind: RealtimeAudioDeviceKind | str) -> SupportedStreamConfig:
    try:
...
def missing_device_error(kind: RealtimeAudioDeviceKind | str, configured_name: str | None) -> str:
    device_kind = _kind(kind)
