...
class AgentMarkdownCell:
    ...
    def display_hyperlink_lines(self, width: int) -> list[HyperlinkLine]:
        ...
        width = int(width)
        if width <= LIVE_PREFIX_COLS:
            return prefix_hyperlink_lines(
                [HyperlinkLine.new("")],
                ASSISTANT_PREFIX,
                ASSISTANT_CONTINUATION_PREFIX,
            )
        rendered = render_markdown_agent_with_links_and_cwd(
...
def run_terminal_user_prompt_output(
    ...
    *,
    ...
    write_source_cell: Callable[..., Any] | None = None,
) -> None:
    """Write a terminal user prompt through the message history-cell boundary."""

    clear_live_status()
    if write_source_cell is not None:
        write_source_cell(
...
class TerminalUserPromptOutputWriter:
    ...
    def write(self, message: str) -> None:
        run_terminal_user_prompt_output(
            ...
            write_source_cell=self.write_source_cell,
        )
