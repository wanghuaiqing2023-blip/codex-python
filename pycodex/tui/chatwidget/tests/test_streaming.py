...
def test_reasoning_final_without_summary_is_noop() -> None:
    # Rust parity: ChatWidget::on_agent_reasoning_final only inserts a
    # reasoning summary history cell when the accumulated buffer is non-empty.
    state = StreamingWidgetState()
    state.on_agent_reasoning_final()
...
def test_agent_message_completion_restore_flag_depends_on_phase() -> None:
    ...
    assert commentary.status_indicator_visible
    assert not commentary.status_state.pending_status_indicator_restore


def test_commentary_completion_restores_working_status_after_stream_idle() -> None:
    # Rust sources:
    # - chatwidget/streaming.rs::on_agent_message_item_completed always calls
    #   maybe_restore_status_indicator_after_stream_idle after setting the
    #   commentary restore flag.
    # - chatwidget/tests/exec_flow.rs::preamble_keeps_working_status_snapshot.
    state = StreamingWidgetState(task_running=True)
    state.on_agent_message_delta("Preamble line\n")
    state.on_commit_tick()

    assert state.status_indicator_visible is False
    state.on_agent_message_item_completed("Preamble line\n", MessagePhase.Commentary)
