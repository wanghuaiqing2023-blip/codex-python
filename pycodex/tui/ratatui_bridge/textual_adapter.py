"""Adapters from ratatui_bridge semantic render output to vendored Textual/Rich.

The bridge owns portable layout and buffer semantics. Textual owns real terminal
I/O. This module is the narrow handoff layer between the two.
"""
...
def buffer_to_rich_text(buffer: Buffer, area: Optional[Rect] = None, trim_end: bool = False):
    ...
    The resulting Rich object is still just a renderable value. It performs no
    terminal I/O; Textual decides how and when it reaches the real terminal.
    """
    from pycodex.tui.textual_compat import Text as RichText
...
def buffer_to_plain_text(buffer: Buffer, area: Optional[Rect] = None, trim_end: bool = False) -> str:
    ...
    for y in range(target.y, target.bottom()):
        text = "".join(buffer.cell(x, y).symbol for x in range(target.x, target.right()))
...
def _trim_cells_right(cells: List[Cell]) -> List[Cell]:
    ...
    while end > 0 and cells[end - 1].symbol == " ":
        end -= 1
    return cells[:end]
