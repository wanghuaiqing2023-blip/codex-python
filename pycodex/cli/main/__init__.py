...
from pycodex.cli.doctor import (
...
from ..marketplace_cmd import run as _run_marketplace_command
from ..plugin_cmd import (
    help_text as _plugin_help_text,
    parse_args as _parse_plugin_args,
    run_plugin_add,
    run_plugin_list,
    run_plugin_remove,
)
from ..remote_control_cmd import help_text as _remote_control_help_text, parse_args as _parse_remote_control_args, run as _run_remote_control_command
    ...
    *,
    ...
    if subcommand == "marketplace":
        try:
            ...
            return 2
        return _run_marketplace_command(
            command_args,
    ...
    handlers = {
        "add": run_plugin_add,
        "list": run_plugin_list,
        "remove": run_plugin_remove,
    }
    handler = handlers.get(subcommand)
    if handler is None:
        print(f"Unrecognized plugin subcommand: {subcommand}", file=stderr)
        return 64
    return handler(command_args, stdout=stdout, stderr=stderr)
...
def main(
    ...
    if parsed.is_interactive:
        ...
        if parsed.command_args and any(arg in {"-h", "--help"} for arg in parsed.command_args):
            ...
            return 0
        return _run_app_server_command(
            parsed.command_args,
            stdout=out,
            stderr=err,
...
def _run_tui(
    ...
    *,
    ...
    terminal_runtime = app_runtime if startup_session_kwargs else active_thread_runtime
    exit_code = run_terminal_tui(
        active_thread_runtime=terminal_runtime,
        stdout=out,
        stdin=tui_stdin,
        exit_info_sink=exit_info.append,
    )
    if exit_code == 0 and exit_info:
        _write_tui_exit_summary(exit_info[-1], out)
    return exit_code


def _write_tui_exit_summary(exit_info: object, stdout: TextIO) -> None:
    """Format the TUI result at the Rust ``codex-cli::main`` boundary."""

    from pycodex.tui.main import format_exit_messages

    for line in format_exit_messages(exit_info):
...
def _run_app_server_command(
    ...
    *,
    ...
    if not args:
        listen = "stdio://"
        if "--listen" in command_args:
            listen = command_args[command_args.index("--listen") + 1]
        if listen != "stdio://":
            print(f"pycodex: app-server transport is not implemented: {listen}", file=stderr)
            return 2
        from pycodex.app_server.main import AppServerArgsProjection, run

        try:
            asyncio.run(
                run(
                    arg0_paths=SimpleNamespace(codex_self_exe=Path(sys.executable)),
            ...
            print(f"pycodex: app-server failed: {exc}", file=stderr)
            return 1
        return 0
...
def _run_doctor(*, parsed: "ParsedCli", stdout: TextIO, stderr: TextIO) -> int:
    ...
    checks["system"] = _timed_doctor_mapping(lambda: system_check().to_mapping())
    ...
    checks["runtime"] = _timed_doctor_mapping(
        lambda: runtime_check(
            current_version=__version__,
            codex_home=codex_home_path,
        ).to_mapping()
    )
    checks["search"] = _timed_doctor_mapping(lambda: search_check(codex_home=codex_home_path).to_mapping())
    ...
    if config_error is None and codex_home_path is not None:
        ...
        checks["terminal_title"] = _timed_doctor_mapping(
            lambda: terminal_title_check(cwd=Path.cwd(), config=dict(config)).to_mapping()
        )
        ...
        checks["background_server"] = _timed_doctor_mapping(
            lambda: background_server_check(codex_home=codex_home_path).to_mapping()
        )
        checks["thread_inventory"] = _timed_doctor_mapping(
            lambda: thread_inventory_check(
                codex_home=codex_home_path,
    ...
    checks["git"] = _timed_doctor_mapping(lambda: git_check(cwd=Path.cwd()).to_mapping())
...
def _doctor_updates_mapping(
    *,
    ...
    return updates_check_from_config(
        config,
        codex_home=codex_home,
        current_version=__version__,
    ).to_mapping()
