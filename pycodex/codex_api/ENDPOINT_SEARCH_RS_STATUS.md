# codex-api/src/endpoint/search.rs status

Rust module: `codex/codex-rs/codex-api/src/endpoint/search.rs`

Python module: `pycodex/codex_api/endpoint/search.py`

Status: `complete`

Ported contract:

- `SearchClient` owns an injected transport, provider, auth provider, and
  optional telemetry placeholder matching the Rust client shape.
- `SearchClient.path()` returns `alpha/search`.
- `search` posts typed `SearchRequest` JSON to `alpha/search`.
...
- Search endpoint responses decode through `SearchResponse`.
- Search request encode failures and response decode failures map to
  `ApiError.stream(...)` with operation-specific messages.
