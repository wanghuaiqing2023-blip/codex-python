...
| `src/mitm.rs` | `pycodex.network_proxy` | partial | MITM policy blocking, method clamp, host mismatch, local/private recheck, hook match/miss behavior, blocked telemetry, hook action header replacement, and request Host/authority/URI/path helper contracts are covered by Rust-derived/source-derived tests. TLS termination, body inspection stream, and upstream forwarding remain open. |
| `src/socks5.rs` | `pycodex.network_proxy` | partial | Dependency-light SOCKS5 TCP/UDP policy inspection covers proxy-disabled, limited-mode, MITM-required, blocked telemetry, and non-domain audit event behavior from Rust tests. The stdlib SOCKS5 TCP no-auth CONNECT listener/relay and UDP ASSOCIATE/relay slices are covered by real local socket tests. Native Rama identity and full task orchestration remain open. |
| `src/certs.rs` | `pycodex.network_proxy` | partial | Managed MITM CA path shape, Unix key-file validation, and atomic create-new file persistence are covered by Rust-derived/source-derived tests. Real CA/host certificate generation and rustls acceptor setup remain open. |

## Known Gaps

- No full live HTTP/SOCKS proxy server runtime. The HTTP/1 CONNECT listener
  direct-tunnel/upstream-proxy route, plain HTTP direct/upstream forwarding,
  SOCKS5 TCP no-auth CONNECT relay, and SOCKS5 UDP ASSOCIATE/relay slices
  are covered with real local socket tests; `NetworkProxy.run` also starts
  and shuts down stdlib HTTP/SOCKS listener tasks from reserved listeners.
- No MITM TLS/certificate generation runtime; only the managed CA file-safety slice is implemented.
- No Rama/rustls/tokio runtime identity.
- `runtime.rs` uses on-demand reload rather than a standalone reload task; host policy, DNS/private-address guard behavior, domain-list mutation, unix-socket checks, runtime accessors, and reload state replacement are covered through dependency-light state boundaries with injectable tests.
- No native `tokio::spawn`/JoinHandle identity for `src/proxy.rs`; stdlib task startup/shutdown semantics are covered.
- Full runtime `ConfigState` internals are dependency-light and do not include Rust MITM state objects or live reload/runtime handles.

These are crate-level gaps, so `codex-network-proxy` is `module_progress`, not
strict `complete`.
