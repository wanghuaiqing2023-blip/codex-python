"""Handler for the Rust ``list_mcp_resource_templates`` module."""
...
from . import ListResourceTemplatesArgs
from . import ListResourceTemplatesPayload
...
class ListMcpResourceTemplatesHandler:
    ...
    def tool_name(self) -> ToolName:
        return ToolName.plain(mcp_resource_spec.LIST_MCP_RESOURCE_TEMPLATES_TOOL_NAME)

    def spec(self) -> dict[str, JsonValue]:
        return mcp_resource_spec.create_list_mcp_resource_templates_tool()
    ...
    def handle(self, invocation_or_payload: Any) -> FunctionToolOutput:
        payload = _function_payload(
            ...
            mcp_resource_spec.LIST_MCP_RESOURCE_TEMPLATES_TOOL_NAME,
        )
        arguments = parse_mcp_resource_arguments(payload.arguments)
        args = ListResourceTemplatesArgs.from_mapping(arguments)
        ...
        try:
            if server is None:
                ...
                result = ListResourceTemplatesPayload.from_all_servers(
                    _provider_call(
                        self.provider.list_all_resource_templates,
                        "resources/templates/list",
                    )
                )
            else:
                result = ListResourceTemplatesPayload.from_single_server(
                    server,
                    _provider_call(
                        lambda: self.provider.list_resource_templates(server, cursor),
                        "resources/templates/list",
                    ),
                )
...
__all__ = ["ListMcpResourceTemplatesHandler"]
