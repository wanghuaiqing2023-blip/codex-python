...
def _respond_to_model(message: str) -> Exception:
    # Import lazily because Python initializes this package before any child
    # handler; the router also imports handler-adjacent Core modules.
...
def parse_arguments(arguments: str, parser: Callable[[JsonValue], _T] | None = None) -> JsonValue | _T:
    try:
        ...
        raise _respond_to_model(f"failed to parse function arguments: {err}") from err
    return parser(value) if parser is not None else value


def updated_hook_command(updated_input: JsonValue) -> str:
    if not isinstance(updated_input, dict):
        raise _respond_to_model("hook returned updatedInput without string field `command`")
...
def rewrite_function_arguments(
    ...
    if not isinstance(value, dict):
        raise _respond_to_model(f"{tool_name} arguments must be an object")
    rewrite(value)
    try:
        return json.dumps(value, ensure_ascii=False, separators=(",", ":"))
    except (TypeError, ValueError) as err:
        raise _respond_to_model(f"failed to serialize rewritten {tool_name} arguments: {err}") from err
...
def resolve_tool_environment(turn: Any, environment_id: str | None) -> Any | None:
    ...
    for environment in candidates or ():
        if getattr(environment, "environment_id", None) == environment_id:
            return environment
    raise _respond_to_model(f"unknown turn environment id `{environment_id}`")
...
_HANDLER_EXPORTS = {
    "ApplyPatchHandler": ("pycodex.core.tools.handlers.apply_patch", "ApplyPatchHandler"),
