"""Concrete session state and behavior owned by ``session::session``."""
...
from pycodex.core.guardian import GuardianReviewSessionManager
...
from pycodex.core.state.turn import ActiveTurn, PendingRequestPermissions, TurnState
from pycodex.core.session.input_queue import InputQueue
from pycodex.core.session import inject as session_inject
from pycodex.core.session import mcp as session_mcp
...
def _resolve_pending_user_input(future: asyncio.Future[Any], response: Any) -> None:
    if not future.done():
...
class Session:
    ...
    guardian_review_session: GuardianReviewSessionManager = field(
        default_factory=GuardianReviewSessionManager
    )
    ...
    active_turn: ActiveTurn | None = field(default_factory=ActiveTurn)
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.guardian_review_session, GuardianReviewSessionManager):
            raise TypeError("guardian_review_session must be GuardianReviewSessionManager")
        ...
        if not isinstance(self.input_queue, InputQueue):
            raise TypeError("input_queue must be InputQueue")
        if self.active_turn is not None and not isinstance(self.active_turn, ActiveTurn):
            raise TypeError("active_turn must be ActiveTurn or None")
    ...
    async def new_default_turn(self) -> TurnContext:
    ...
    async def active_turn_context(self) -> TurnContext | None:
    ...
    async def active_turn_context_and_cancellation_token(self) -> tuple[TurnContext, Any] | None:
        active_turn = self.active_turn
        task = None if active_turn is None else active_turn.task
        if task is None:
            return None
        cancellation_token = getattr(task, "cancellation_token", None)
        child_token = getattr(cancellation_token, "child_token", None)
        if callable(child_token):
            cancellation_token = child_token()
        return getattr(task, "turn_context"), cancellation_token

    async def mark_turn_context_active(self, turn_context: TurnContext) -> None:
        if self.active_turn is None:
            self.active_turn = ActiveTurn()
        self.active_turn.task = SimpleNamespace(turn_context=turn_context)

    async def clear_turn_context_active(self, turn_context: TurnContext) -> None:
        active_turn = self.active_turn
        task = None if active_turn is None else active_turn.task
    ...
    async def record_context_updates_and_set_reference_context_item(self, turn_context: TurnContext) -> None:
    ...
    inject_no_new_turn = session_inject.inject_no_new_turn
    inject_if_running = session_inject.inject_if_running
    mcp_elicitation_reviewer = session_mcp.mcp_elicitation_reviewer
    request_mcp_server_elicitation = session_mcp.request_mcp_server_elicitation
    resolve_elicitation = session_mcp.resolve_elicitation
    list_resources = session_mcp.list_resources
    list_resource_templates = session_mcp.list_resource_templates
    read_resource = session_mcp.read_resource
    call_tool = session_mcp.call_tool
    refresh_mcp_servers_if_requested = session_mcp.refresh_mcp_servers_if_requested
    refresh_mcp_servers_now = session_mcp.refresh_mcp_servers_now
    cancel_mcp_startup = session_mcp.cancel_mcp_startup
    ...
    async def emit_turn_item_started(self, turn_context: TurnContext, item: TurnItem) -> None:
    ...
    async def emit_turn_item_completed(self, turn_context: TurnContext, item: TurnItem) -> None:
        if not isinstance(item, TurnItem):
            item = TurnItem.from_mapping(item)
        await self.send_event(
            turn_context,
    ...
    async def clone_history(self) -> SessionHistory:
        return SessionHistory(list(self.history))
    ...
    async def send_event(self, turn_context: TurnContext, event: EventMsg | dict[str, Any]) -> None:
    ...
    async def request_user_input(
        ...
        """Emit a user-input request and wait for the matching turn response."""
        ...
        sub_id = str(
            getattr(turn_context, "sub_id", None)
            or getattr(turn_context, "turn_id", None)
            or ""
        )
        loop = asyncio.get_running_loop()
        future: asyncio.Future[RequestUserInputResponse | None] = loop.create_future()
        state = self.active_turn.turn_state
        with state.pending_user_input_lock:
            previous = state.pending_user_input.get(sub_id)
            state.pending_user_input[sub_id] = (loop, future)
        if previous is not None:
            previous_loop, previous_future = previous
            previous_loop.call_soon_threadsafe(
                _resolve_pending_user_input,
                previous_future,
                None,
            )
        ...
        try:
            ...
            with state.pending_user_input_lock:
                current = state.pending_user_input.get(sub_id)
                if current is not None and current[1] is future:
                    state.pending_user_input.pop(sub_id, None)

    async def notify_user_input_response(
        self,
        sub_id: str,
        response: RequestUserInputResponse | Mapping[str, Any],
    ) -> None:
        """Resolve the Rust-style pending user-input channel for one turn."""

        if not isinstance(sub_id, str):
            raise TypeError("sub_id must be a string")
        if not isinstance(response, RequestUserInputResponse):
            to_mapping = getattr(response, "to_mapping", None)
            response = RequestUserInputResponse.from_mapping(
                to_mapping() if callable(to_mapping) else response
            )
        state = self.active_turn.turn_state
        with state.pending_user_input_lock:
            pending = state.pending_user_input.pop(sub_id, None)
        if pending is None:
            return
        loop, future = pending
        loop.call_soon_threadsafe(_resolve_pending_user_input, future, response)
    ...
    async def send_token_count_event(self, turn_context: TurnContext) -> None:
    ...
    async def record_token_usage_info(self, turn_context: TurnContext, token_usage: TokenUsage | None) -> None:
...
__all__ = ["Session", "SessionHistory"]
...
def _turn_id_for_event(turn_context: TurnContext) -> str:
...
def _turn_context_item_from_turn_context(turn_context: TurnContext) -> TurnContextItem:
...
async def _load_turn_skills(session: Session, config: Any) -> Any:
...
async def _load_plugins_for_config(session: Session, config: Any) -> Any:
...
def _session_connectors(session: Session) -> tuple[Any, ...]:
    connectors = getattr(session, "available_connectors", ())
    return tuple(connectors or ())


def _apps_enabled(turn_context: TurnContext) -> bool:
