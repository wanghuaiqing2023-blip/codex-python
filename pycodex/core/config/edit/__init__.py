"""Config edit engine aligned with ``core/src/config/edit.rs``."""
...
def _add_tool_suggest_disabled_tool(
    ...
    candidates = existing_disabled if isinstance(existing_disabled, list) else ()
    for raw_candidate in candidates:
        candidate = document_helpers.parse_tool_suggest_disabled_tool(raw_candidate)
    ...
    value = document_helpers.tool_suggest_disabled_tools_value(disabled_tools)
...
def _replace_mcp_servers(config: MutableMapping[str, Any], servers: Mapping[str, Any]) -> bool:
    ...
    serialized = {
        str(name): document_helpers.serialize_mcp_server(server)
