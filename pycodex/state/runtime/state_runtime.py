...
from .backfill import ensure_backfill_state_row
from .memories import clear_memory_data_in_connection
...
class StateRuntime:
    ...
    async def query_logs(self, query: Any = None) -> list[Any]:
        return await self.logs.query_logs(query)

    async def max_log_id(self, query: Any = None) -> int:
        return await self.logs.max_log_id(query)
