...
7. 更新 `PORTING_STATUS.md`。
8. 在 `porting_notes/turns/` 写本轮记录。
9. 向用户汇报 Rust 坐标、Python 坐标、完成内容、验证证据和剩余缺口。
...
7. 本轮记录已经写入 `porting_notes/turns/`。
...
8. 更新 `PORTING_STATUS.md`。
9. 在 `porting_notes/turns/` 写本轮记录。
10. 向用户说明：本轮完成了什么、对应 Rust 坐标是什么、测试证据是什么、还剩什么。
...
- 重要进展必须更新 `PORTING_STATUS.md` 和 `porting_notes/turns/`。
...
请以 Rust 源码树为权威坐标，以 `codex/.understand-anything/knowledge-graph.json` 为导航索引。每轮选择一个最小的 crate/module/function 行为切片，先确认 Rust 源码行为，再映射或修正 Python 实现，然后运行 focused validation，并更新 `PORTING_STATUS.md` 与 `porting_notes/turns/`。
