"""Low-level Rust/Python TUI E2E comparison implementation.
...
This E2E support layer intentionally starts with pipe/subprocess comparisons.  It can
...
def _windows_pe_stack_reserve(path: Path) -> int | None:
    try:
        with path.open("rb") as handle:
            ...
            handle.seek(optional_header + 72)
            if magic == 0x20B:
...
def _set_windows_pe_stack_reserve(path: Path, reserve: int) -> None:
    with path.open("r+b") as handle:
        ...
        handle.seek(optional_header + 72)
        if magic == 0x20B:
            handle.write(struct.pack("<Q", int(reserve)))
            return
        if magic == 0x10B:
            handle.write(struct.pack("<I", int(reserve)))
            return
        raise ValueError(f"unsupported PE optional-header magic 0x{magic:x}: {path}")


def _native_codex_with_e2e_stack(executable: Path) -> Path:
    """Keep deep debug-only TUI paths from overflowing the PE main stack."""

    if os.name != "nt" or not executable.is_file():
        return executable
    reserve = _windows_pe_stack_reserve(executable)
    if reserve is None or reserve >= _NATIVE_E2E_STACK_RESERVE:
        return executable

    target = executable.with_name(f"{executable.stem}.e2e-stack{executable.suffix}")
    if (
        target.is_file()
        and target.stat().st_mtime_ns >= executable.stat().st_mtime_ns
        and (_windows_pe_stack_reserve(target) or 0) >= _NATIVE_E2E_STACK_RESERVE
    ):
        return target
    ...
    try:
        _set_windows_pe_stack_reserve(temporary, _NATIVE_E2E_STACK_RESERVE)
...
def build_inline_tui_command(
    ...
    *,
    ...
    if kind == "rust":
        ...
        exe = _native_codex_with_e2e_stack(Path(exe))
...
def _write_windows_conpty_text(
    handle: object,
    text: str,
    *,
    chunk_delay: float,
    ...
    chunks = [text] if atomic_write else _conpty_input_chunks(text)
    for chunk in chunks:
        _timing_trace(
...
def _wait_for_windows_conpty_screen_text(
    chunks: Sequence[bytes],
    needle: str,
    *,
    timeout: float,
    ...
    """Wait until text is visible on the reconstructed current VT screen.

    Ratatui diff draws preserve unchanged cells with cursor movement instead
    of rewriting their characters, so transient status checks must inspect the
    reconstructed screen rather than ANSI-stripped cumulative output.
    """
        ...
        for step in steps or ():
            if (
                ...
                if step.ready_pattern is not None:
                    ...
                    ready = _wait_for_windows_conpty_screen_text(
                        output_chunks,
                        step.ready_screen_text,
                        timeout=float(step.ready_timeout),
                        size=active_size,
                    )
            ...
            if step.text:
                ...
                _write_windows_conpty_text(
                    input_write,
                    step.text,
                    chunk_delay=float(step.chunk_delay),
                    atomic_write=bool(step.atomic_write),
                )
