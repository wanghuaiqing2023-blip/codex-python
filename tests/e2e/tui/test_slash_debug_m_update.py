"""End-to-end coverage for Rust's ``/debug-m-update`` slash command."""
...
def test_debug_m_update_registry_and_effect_contract() -> None:
    # Rust slash_commands::slash_memory_update_reports_stubbed_feature fixes
    # both the local error text and the absence of a memory operation.
    route = terminal_slash_command_routes()[SlashCommand.MEMORY_UPDATE]

    assert SlashCommand.MEMORY_UPDATE.command() == "debug-m-update"
    assert SlashCommand.MEMORY_UPDATE.supports_inline_args() is False
    assert SlashCommand.MEMORY_UPDATE.available_during_task() is False
    assert SlashCommand.MEMORY_UPDATE.available_in_side_conversation() is False
    assert route.category == "core"
    assert route.outcome == "effect"
    assert route.python_owner == "pycodex.tui.chatwidget.slash_dispatch"


def test_windows_conpty_native_and_python_debug_m_update_reports_tui_stub(
    tmp_path: Path,
) -> None:
