"""End-to-end coverage for the ``/exit`` slash command."""
...
def test_exit_slash_command_uses_local_exit_effect() -> None:
    # Rust: chatwidget::slash_dispatch handles Quit | Exit through
    # request_quit_without_confirmation.
    route = terminal_slash_command_routes()[SlashCommand.EXIT]

    assert route.outcome == "effect"
    assert plan_terminal_local_command("/exit").action == "exit"


def test_windows_conpty_native_and_python_exit_clean_shutdown_when_enabled(
    tmp_path: Path,
) -> None:
