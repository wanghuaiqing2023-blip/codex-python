...
def test_menu_editor_resume_and_clear_plans_match_rust_branches() -> None:
    goal = ThreadGoal(status=ThreadGoalStatus.Paused, objective="Resume me")

    assert open_thread_goal_menu_plan(None).actions == ("add_info_message",)
    assert open_thread_goal_menu_plan(goal).actions == ("show_goal_summary",)
    assert open_thread_goal_editor_plan(None).actions == ("add_error_message", "add_info_message")
    assert open_thread_goal_editor_plan("thread", goal).actions == ("show_goal_edit_prompt",)

    resume = maybe_prompt_resume_paused_goal_after_resume_plan("thread", goal)
...
def test_set_thread_goal_objective_replace_confirmation_and_success_plans() -> None:
    active = ThreadGoal(status=ThreadGoalStatus.Active)
    completed = ThreadGoal(status=ThreadGoalStatus.Complete)

    confirm = set_thread_goal_objective_plan("thread", "New objective", ThreadGoalSetMode.ConfirmIfExists, active)
...
def test_set_thread_goal_status_and_error_plans() -> None:
    updated = set_thread_goal_status_plan("thread", ThreadGoalStatus.Blocked, response_goal=ThreadGoal(ThreadGoalStatus.Blocked, tokens_used=5))
    assert updated.actions == ("thread_goal_set", "add_info_message")
    assert updated.message == "Goal blocked"
    assert updated.hint == "5 tokens used"

    err = RuntimeError("server disappeared")
    failed = set_thread_goal_objective_plan("thread", "x", ThreadGoalSetMode.ReplaceExisting, set_error=err)
