...
class DeviceAuthTests(unittest.TestCase):
    ...
    def test_request_device_auth_user_code_success(self) -> None:
        ...
        with patch("pycodex.cli.main.urlopen", return_value=_FakeTextResponse(json.dumps(payload))):
    ...
    def test_request_device_auth_user_code_http_error(self) -> None:
        ...
        with patch("pycodex.cli.main.urlopen", side_effect=error):
    ...
    def test_poll_device_auth_token_retries_on_forbidden(self) -> None:
        ...
        stderr = io.StringIO()
        with patch("pycodex.cli.main._DEVICE_AUTH_MAX_WAIT_SECONDS", 1.0):
            with patch("pycodex.cli.main.urlopen", side_effect=_http_getter):
                with patch("pycodex.cli.main.time.sleep"):
                    code_payload = _poll_device_auth_token(
                        "https://auth.example.com/api/accounts",
                        "device-id",
                        "ABC-123",
    ...
    def test_poll_device_auth_token_retries_and_times_out_if_no_authorization(self) -> None:
        ...
        stderr = io.StringIO()
        with patch("pycodex.cli.main._DEVICE_AUTH_MAX_WAIT_SECONDS", 0.0):
            with patch("pycodex.cli.main.urlopen", side_effect=_empty_payload):
                with patch("pycodex.cli.main.time.sleep"):
                    with self.assertRaisesRegex(
                        RuntimeError,
    ...
    def test_poll_device_auth_token_invalid_json(self) -> None:
        with patch("pycodex.cli.main.urlopen", return_value=_FakeTextResponse("not-json")):
    ...
    def test_poll_device_auth_token_times_out(self) -> None:
        ...
        stderr = io.StringIO()
        with patch("pycodex.cli.main._DEVICE_AUTH_MAX_WAIT_SECONDS", 0.0):
            with patch("pycodex.cli.main.urlopen", side_effect=_forbidden):
                with patch("pycodex.cli.main.time.sleep"):
                    with self.assertRaisesRegex(
                        RuntimeError,
                        "device auth timed out after 15 minutes",
    ...
    def test_run_device_auth_login_success(self) -> None:
        ...
        with patch("pycodex.cli.main._request_device_auth_user_code") as request_user_code:
            ...
            with patch(
                "pycodex.cli.main._poll_device_auth_token",
                return_value={"authorization_code": "auth-code", "code_verifier": "code-verifier"},
            ) as poll_code:
                with patch(
                    "pycodex.cli.main._exchange_device_auth_code",
                    return_value={
                        "access_token": "access-token",
                        "refresh_token": "refresh-token",
                        "id_token": id_token,
                    ...
                    with patch("pycodex.cli.main.write_auth_json", side_effect=lambda auth: captured.append(auth)):
    ...
    def test_run_device_auth_login_request_user_code_error(self) -> None:
        stderr = io.StringIO()
        with patch(
            "pycodex.cli.main._request_device_auth_user_code",
